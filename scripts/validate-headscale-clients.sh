...
client_socket_path() {
  local client_name="$1"
  echo "${HEADSCALE_VALIDATE_DIR}/${client_name}/tailscaled.sock"
}
...
HEADSCALE_PREAUTH_KEY="$(
  headscale_ssh \
    "sudo headscale preauthkeys create --reusable --expiration ${HEADSCALE_VALIDATE_KEY_EXPIRATION} --tags ${HEADSCALE_VALIDATE_TAG}" \
