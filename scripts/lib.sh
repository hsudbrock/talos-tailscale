...
load_env() {
  ...
  : "${HEADSCALE_AUTH_KEY:=}"
  : "${HEADSCALE_AUTH_KEY_FILE:=$(state_path headscale/talos-authkey)}"
  : "${HEADSCALE_TALOS_USER:=headscale-talos}"
  if [[ -z "${HEADSCALE_TALOS_TAG+x}" ]]; then
    HEADSCALE_TALOS_TAG="tag:talos-lab"
  fi
  : "${HEADSCALE_TALOS_KEY_EXPIRATION:=720h}"
  : "${HEADSCALE_HOST_CONNECT_URL:=http://127.0.0.1:${HEADSCALE_HOST_HTTP_PORT}}"
  : "${HEADSCALE_HOST_AUTH_KEY:=}"
  : "${HEADSCALE_HOST_AUTH_KEY_FILE:=$(state_path headscale/host-authkey)}"
